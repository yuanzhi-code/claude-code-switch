...
function launch(profile, args = []) {
  return new Promise((resolve, reject) => {
    ...
    const child = exec(cmd, { env }, (error, stdout, stderr) => {
      if (error) {
        // Unix-like systems use code 127, Windows uses code 1 for command not found
        const isCommandNotFound = error.code === 127 || (error.code === 1 && error.message.includes('not found'));
        if (isCommandNotFound) {
          return reject(new Error(
            'claude-code command not found. Please install Claude Code first using: npm install -g @anthropic-ai/claude-code'
          ));
        }
        return reject(error);
      }
    ...
    // Handle stdin piping with proper events to avoid race conditions
    child.on('spawn', () => {
