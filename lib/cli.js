#!/usr/bin/env node

const path = require('path');
const os = require('os');
const { loadConfig, getProfile, listProfiles, initializeConfig, deleteProfile, setDefault } = require('./config');
const { launch, validateProfile } = require('./launcher');
const { addProfileInteractive } = require('./add');
const ui = require('./ui');

// Read version from package.json
const PACKAGE_JSON = require('../package.json');
const VERSION = PACKAGE_JSON.version;

// Config directory
const CONFIG_DIR = path.join(os.homedir(), '.ccswitch');
const CONFIG_FILE = path.join(CONFIG_DIR, 'profiles.json');

/**
 * Print error message and exit
 * @param {string} message - Error message
 * @param {number} code - Exit code
 */
function fatalError(message, code = 1) {
  console.error(`ccswitch: ${message}`);
  process.exit(code);
}

/**
 * Show help message
 */
function showHelp() {
  console.log('Usage: ccswitch [profile] [options]');
  console.log('       ccswitch add <name>');
  console.log('       ccswitch delete <name>');
  console.log('       ccswitch set-default <name>');
  console.log('');
  console.log('Arguments:');
  console.log('  profile    Name of profile to use (default: uses \'default\' from config)');
  console.log('');
  console.log('Commands:');
  console.log('  add <name>         Interactively add a new profile');
  console.log('  delete <name>      Delete a profile');
  console.log('  set-default <name> Set a profile as the default');
  console.log('');
  console.log('Options:');
  console.log('  --help, -h     Show this help message');
  console.log('  --version, -V  Show version information');
  console.log('  --list, -l     List all available profiles');
  console.log('  --d, -d        Skip permission checks (alias for --dangerously-skip-permissions)');
  console.log('');
  console.log('Examples:');
  console.log('  ccswitch kimi');
  console.log('  ccswitch');
  console.log('  ccswitch kimi --d');
  console.log('  ccswitch add myprofile');
  console.log('  ccswitch delete myprofile');
  console.log('  ccswitch set-default kimi');
}

/**
 * Show version information
 */
function showVersion() {
  console.log(`ccswitch v${VERSION}`);
}

/**
 * Main entry point
 */
async function main() {
  const args = process.argv.slice(2);

  // Expand shorthand flags
  const expandedArgs = args.map(arg => {
    if (arg === '--d' || arg === '-d') {
      return '--dangerously-skip-permissions';
    }
    return arg;
  });

  // Check for help flag
  if (expandedArgs.includes('--help') || expandedArgs.includes('-h')) {
    showHelp();
    process.exit(0);
  }

  // Check for version flag
  if (expandedArgs.includes('--version') || expandedArgs.includes('-V')) {
    showVersion();
    process.exit(0);
  }

  // Check for list flag
  if (expandedArgs.includes('--list') || expandedArgs.includes('-l')) {
    const config = loadConfig(CONFIG_DIR);

    if (!config) {
      console.log('No config file found.');
      console.log('');
      ui.dim(`Config file: ${CONFIG_FILE}`);
      process.exit(0);
    }

    const profiles = listProfiles(config);

    ui.highlight('Available profiles:');
    console.log('');

    if (profiles.length === 0) {
      ui.dim('  (no profiles configured)');
    } else {
      for (const name of profiles) {
        console.log(`  ${ui.formatProfileName(name, config.default)}`);
      }
    }

    console.log('');
    ui.dim(`Config file: ${CONFIG_FILE}`);
    process.exit(0);
  }

  // Check for add command: ccswitch add <name>
  if (expandedArgs[0] === 'add') {
    if (!expandedArgs[1]) {
      console.error('Error: Profile name is required.');
      console.error('');
      console.error('Usage: ccswitch add <name>');
      console.error('');
      console.error('Example: ccswitch add myprofile');
      process.exit(1);
    }
    const profileName = expandedArgs[1];
    await addProfileInteractive(CONFIG_DIR, profileName);
    return;
  }

  // Check for delete command: ccswitch delete <name>
  if (expandedArgs[0] === 'delete') {
    if (!expandedArgs[1]) {
      console.error('Error: Profile name is required.');
      console.error('');
      console.error('Usage: ccswitch delete <name>');
      console.error('');
      console.error('Example: ccswitch delete myprofile');
      process.exit(1);
    }
    const profileName = expandedArgs[1];
    const deleted = deleteProfile(CONFIG_DIR, profileName);
    if (deleted) {
      ui.success(`Profile "${profileName}" deleted.`);
      const config = loadConfig(CONFIG_DIR);
      if (config && config.default) {
        ui.dim(`  Default is now: ${config.default}`);
      }
    } else {
      ui.error(`Profile "${profileName}" not found.`);
      process.exit(1);
    }
    return;
  }

  // Check for set-default command: ccswitch set-default <name>
  if (expandedArgs[0] === 'set-default') {
    if (!expandedArgs[1]) {
      console.error('Error: Profile name is required.');
      console.error('');
      console.error('Usage: ccswitch set-default <name>');
      console.error('');
      console.error('Example: ccswitch set-default kimi');
      process.exit(1);
    }
    const profileName = expandedArgs[1];
    const updated = setDefault(CONFIG_DIR, profileName);
    if (updated) {
      ui.success(`Default profile set to "${profileName}".`);
    } else {
      ui.error(`Profile "${profileName}" not found.`);
      process.exit(1);
    }
    return;
  }

  // Parse arguments: first non-flag argument is the profile name
  let profileName = '';
  const extraArgs = [];

  for (const arg of expandedArgs) {
    if (!arg.startsWith('-') && profileName === '') {
      // First non-flag argument is the profile name
      profileName = arg;
    } else {
      // Everything else (including flags and subsequent args) goes to claude-code
      extraArgs.push(arg);
    }
  }

  try {
    // Load or initialize config
    let config = loadConfig(CONFIG_DIR);

    if (!config) {
      ui.error('Config file not found');
      console.log('');

      const initialized = initializeConfig(CONFIG_DIR);

      if (initialized) {
        ui.info('Initializing default config...');
        ui.success('Created default config');
        console.log('');
        ui.dim('Please edit the config file to add your API configurations:');
        console.log('');
        ui.highlight(CONFIG_FILE);
        console.log('');
        ui.warning('Edit the config file and try again.');
        process.exit(1);
      } else {
        fatalError('Failed to create config file.', 1);
      }
    }

    // Get profile
    const profile = getProfile(config, profileName);

    if (!profile) {
      const available = listProfiles(config);
      const defaultName = config.default || '(no default set)';

      if (available.length === 0) {
        fatalError('No profiles found in config file.', 1);
      }

      const searchName = profileName || defaultName;

      ui.error(`Profile not found: ${searchName}`);
      console.log('');
      ui.info('Available profiles:');
      console.log('');
      for (const name of available) {
        console.log(`  ${ui.formatProfileName(name, config.default)}`);
      }
      console.log('');

      // Similarity check for suggestions
      const searchLower = searchName.toLowerCase();
      const suggestions = available.filter(name => {
        const nameLower = name.toLowerCase();
        return nameLower === searchLower || nameLower.includes(searchLower) || searchLower.includes(nameLower);
      });

      if (suggestions.length > 0 && suggestions[0].toLowerCase() !== searchLower) {
        ui.warning(`Did you mean...?`);
        for (const suggestion of suggestions) {
          console.log(`  ${ui.formatProfileName(suggestion, config.default)}`);
        }
        console.log('');
      }

      fatalError(`Please specify a valid profile.`, 1);
    }

    // Validate profile
    const validation = validateProfile(profile);

    if (!validation.valid) {
      const profileDisplayName = profileName || config.default;
      ui.error(`Profile "${profileDisplayName}" is missing required fields:`);
      console.log('');
      for (const field of validation.missing) {
        ui.dim(`  - ${field}`);
      }
      console.log('');
      ui.info('Required fields:');
      console.log('');
      ui.dim('  ANTHROPIC_AUTH_TOKEN - Your API authentication token');
      ui.dim('  ANTHROPIC_BASE_URL   - Base URL for the API');
      ui.dim('  ANTHROPIC_MODEL      - Model identifier to use');
      console.log('');
      ui.warning(`Solution: Edit ${CONFIG_FILE} and add the missing fields to profile "${profileDisplayName}"`);
      console.log('');
      process.exit(1);
    }

    // Launch Claude Code with parsed extra arguments
    await launch(profile, extraArgs);

  } catch (error) {
    // Ensure exit code is a number
    let exitCode = error.exitCode || error.code || 1;
    if (typeof exitCode !== 'number') {
      exitCode = 1;
    }
    fatalError(error.message, exitCode);
  }
}

// Run
main();
