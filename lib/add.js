...
async function addProfileInteractive(configDir, name) {
  ...
  console.log(`\nAdding profile: ${name}`);
  ui.dim('Press Enter to use default values or skip optional fields.\n');
  ...
  while (!validToken) {
    ...
    if (!ANTHROPIC_AUTH_TOKEN || ANTHROPIC_AUTH_TOKEN.trim() === '') {
      ui.error('API Token is required.');
    } else if (!isValidApiToken(ANTHROPIC_AUTH_TOKEN)) {
      ui.error('API Token must start with "sk-ant-"');
    } else {
      validToken = true;
    }
  }
  ...
  while (!validTimeout) {
    ...
    if (!isValidTimeout(API_TIMEOUT_MS)) {
      ui.error('Timeout must be a positive integer (milliseconds).');
    } else {
      validTimeout = true;
    }
  }
  ...
  try {
    ...
    ui.error(`Failed to add profile: ${error.message}`);
    ui.dim(`Run 'ccswitch add ${name}' to try again.`);
    process.exit(1);
  }
}
